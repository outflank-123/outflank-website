...
interface ProductDetailClientProps {
  product: {
    ...
    branding_config?: { top?: string; left?: string; transform?: string; width?: string; _is_retail?: boolean } | null | any
      ...
      <main className="min-h-screen bg-[#fbfbfd] pt-24 md:pt-28">
        ...
        <div className="max-w-[1400px] mx-auto px-6 pb-2 flex items-center">
          <button
            type="button"
            onClick={handleBack}
            className="inline-flex items-center gap-1.5 text-sm font-semibold text-[#1d1d1f] bg-white border border-black/10 hover:border-black/30 hover:bg-[#f5f5f7] shadow-sm hover:shadow transition-all duration-300 rounded-full px-5 py-2 cursor-pointer group"
          >
            ...
            <div className="lg:col-span-5 flex flex-col pt-2 lg:pt-4">
              ...
              >
                ...
                <div className="flex flex-wrap items-center gap-2 mb-3">
                  {product.categories && (
                    <span className="text-[11px] font-bold uppercase tracking-wider text-[#86868b] bg-[#f5f5f7] px-3 py-1 rounded-full border border-black/5">
                      {product.categories.name}
                    </span>
                  )}
                  {isCustomizable ? (
                    ...
                    <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-neutral-100 text-neutral-600 border border-neutral-200 text-[11px] font-semibold tracking-wide">
                ...
                {/* Mode Toggle or Wholesale Only Indicator */}
                {isRetailAllowed ? (
                  ...
                  <div className="mb-6 inline-flex flex-wrap items-center gap-2 px-4 py-2 rounded-2xl bg-neutral-100 border border-neutral-200/80 text-neutral-900 shadow-2xs">
                    <div className="flex items-center gap-1.5">
                      <Building2 size={14} className="text-neutral-700" />
                      <span className="text-xs font-bold uppercase tracking-wider">Corporate Wholesale Only</span>
                    </div>
                    <span className="text-xs text-neutral-600 font-medium sm:pl-2 sm:border-l border-neutral-300">
                      Wholesale inquiry only • Single-unit retail checkout unavailable
                    </span>
                  </div>
                )}
                ...
                {!isBulkMode ? (
                  ...
                  <div className="mb-6">
                    <div className="text-2xl font-bold text-[#e3231c]">
                      Wholesale Pricing
                    </div>
                    <div className="text-sm text-[#86868b] mt-1">Contact us for massive bulk discounts.</div>
                  </div>
                )}
              </motion.div>
                    ...
                    <div className="flex items-center justify-between mb-4 pb-4 border-b border-black/6">
                      ...
                      <div className="flex items-center bg-[#f5f5f7] rounded-full p-1 border border-black/5">
                        <button 
                          ...
                          className="w-9 h-9 flex items-center justify-center rounded-full bg-white shadow-sm text-[#1d1d1f] hover:bg-[#e3231c] hover:text-white transition-colors cursor-pointer"
                        >
                          <Minus size={15} />
                        </button>
                        <span className="w-10 text-center font-bold text-[#1d1d1f] text-sm">{quantity}</span>
                        <button 
                          type="button"
